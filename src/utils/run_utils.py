...
from src.models import Flor, GatedBN, CTCModel
...
def getModel(config: Configuration, alphabetSize: int = 51) -> Module:
    if config.modelName == ModelName.GATED_BN:
        ...
        return __loadGatedCheckpoint__(model, config)
    elif config.modelName == ModelName.CTC:  # Add this for CTC model
        model = CTCModel(alphabetSize)
        return model
    else:
        raise ValueError(f"Unknown model name: {config.modelName}")
