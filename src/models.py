...
class CTCModel(nn.Module):
    def __init__(self, alphabet_size):
        ...
        self.fc = nn.Linear(256, alphabet_size)  # Output layer

    def forward(self, x):
        # Meratakan input gambar dari dimensi (batch_size, channels, height, width) menjadi (batch_size, sequence_length, input_size)
        # Misalnya, kita anggap height * width menjadi sequence_length, dan channels menjadi input_size
        x = x.view(x.size(0), -1, 128)  # (batch_size, sequence_length, input_size)

        # LSTM layer
        lstm_out, _ = self.lstm(x)  # Output: (batch_size, sequence_length, hidden_size)

        # Mengambil output dari langkah terakhir LSTM
        output = self.fc(lstm_out)  # Output: (N, T, alphabet_size)
